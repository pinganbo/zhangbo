...
void Yin_difference(Yin *yin, uint16_t *buffer)
{
	int16_t i;
	int16_t tau;
	float delta;
...
void Yin_init(Yin *yin, int16_t bufferSize, float threshold)
{
	...
	yin->yinBuffer = (float *)pvPortMalloc(sizeof(float) * yin->halfBufferSize);
...
float Yin_getPitch(Yin *yin, uint16_t *buffer)
{
...
/**
 * Free the autocorellation buffer
 * @param   yin Yin object that has been run over a buffer
 * @return  None
 * @note    Only use when repeated call Yin_init()
 */
void Yin_freeBuffer_RTOS(Yin *yin)
{
	//vPortFree(yin->yinBuffer);
	int16_t i;
	for (i = 0; i < yin->halfBufferSize; i++)
